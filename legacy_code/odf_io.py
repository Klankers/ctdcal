...
def _salt_loader(filename):
    ...
    if (int(saltArray[0][-2]) < 0) | int(saltArray[0][-2]) > 1000:
        ...
        log.warning(
            f"Salt file for {ssscc} has an erroneous standardization dial in the header."
        )
    ...
    if "Reading6" in cols:
        log.warning(
            "More than 5 readings found in salts file - check columns of source."
        )
    ...
    if saltDF["autosalSAMPNO"].value_counts().get("worm", 0) != 2:
        ...
        log.warning(
            f"Suspect number of standards found in salts file {ssscc}. Confirm standardizations."
        )
    ...
    if ((checkvals >= 1.0001 * 2 * k15) & (checkvals <= 0.9999 * 2 * k15)).any():
        ...
        log.warning(
            f"Standards in raw salt file {ssscc} are >0.01 % off from expected K15 value."
        )
    if any(
        (saltDF["BathTEMP"].astype(float) < 17.5)
        | (saltDF["BathTEMP"].astype(float) > 33.5)
    ):
        #   Check bath temperature
        log.warning(
            f"Raw salt file for {ssscc} contains bath temperatures outside of operational ranges."
        )
    ...
    if flagged.any():
        ...
        questionable["cast_id"] = [
            str(s).zfill(3)[-3:] + str(c).zfill(2)
            for s, c in zip(
                saltDF.loc[flagged, "STNNBR"], saltDF.loc[flagged, "CASTNO"]
            )
        ]
...
def remove_autosal_drift(saltDF, refDF):
    ...
    if refDF.shape != (2, 2):
        ...
        if any(abs(diff["CRavg"] > 0.0001)):
            ...
            log.warning(
                f"Salt run at station {saltDF['STNNBR'].iloc[0]} had a CR drift in excess of 0.0001."
            )
        if any(diff["IndexTime"] > 43200):
            #   Warn analyst that over 12 hours passed between standardizations
            log.warning(
                f"Salt run at station {saltDF['STNNBR'].iloc[0]} had >12 hours between standardizations."
            )
        if any(saltDF["IndexTime"].diff().sum() > (diff["IndexTime"])):
            ...
            log.warning(
                f"Salt run at station {saltDF['STNNBR'].iloc[0]} has misaligned standards and samples."
            )
...
def _salt_exporter(
    ...
    if len(stations) > 1:
        log.info(
            "Multiple stations found in salt source file. Writing out .CSVs seperately."
        )
