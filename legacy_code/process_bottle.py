...
def _reft_loader(ssscc, reft_dir=cfg.dirs["reft"]):
    ...
    with open(reft_path, "r", newline="") as f:
        ...
        for row in reftF:
            if len(row) != 17:  # skip over 'bad' rows (empty lines, comments, etc.)
                if len(row) > 17:
                    log.warning(
                        f"Raw REFT for {ssscc} includes line with abnormally large number of columns. Check .CAP file."
                    )
                else:
                    continue
    ...
    )  #   Opt in to future downcasting
    reftDF = reftDF.replace(to_replace=["bn", "diff", "val", "t90", "="], value=np.nan)
    reftDF = reftDF.dropna(axis=1)
    reftDF.loc[:, 1] = reftDF[[1, 2, 3, 4]].agg(
        " ".join, axis=1
    )  # dd/mm/yy/time cols are
    reftDF = reftDF.drop(columns=[2, 3, 4])  # read separately; combine into one

    if len(reftDF.columns) != 6:
        log.warning(
            "Raw REFT file for {ssscc} has the incorrect number of columns. Check .CAP file."
        )
    ...
    reftDF["datetime"] = pd.to_datetime(
        reftDF["datetime"], format="%d %b %Y %H:%M:%S"
    )  #   Datetime checks, otherwise col is not used

    #   Check contents of the file for stuff that the analyst should double-check (don't immediately flag)
    if any(
        (
            (reftDF["datetime"] < pd.Timestamp("1985-01-01"))
            | (reftDF["datetime"] > pd.Timestamp.today())
        )
    ):
        #   Check timestamps
        log.warning(
            f"Raw REFT file for {ssscc} contains erroneous timestamps. Check instrument configuration."
        )
    if any((reftDF["raw_value"] < 50000.0) | (reftDF["raw_value"] > 1000000)):
        #   Check frequencies
        log.warning(
            f"Raw REFT file reports potentially erroneous frequencies for SSSCC {ssscc}."
        )
    if any((reftDF["T90"] < -2) | (reftDF["T90"] > 40)):
        #   Check calibration coeffs based on frequencies
        log.warning(
            f"Raw REFT file reports potentially erroneous temperatures for SSSCC {ssscc}. Check instrument frequencies and calibration coefficients."
        )
    if any((reftDF["btl_fire_num"] == 1) & (reftDF["btl_fire_num"].shift(1) > 1)):
        #   Check bottle column incrementations
        log.warning(
            f"Raw REFT file has bottle reset to 1 during {ssscc}. Check .CAP file for tests or other casts."
        )
        ...
        log.warning(
            f"Raw REFT for {ssscc} contains duplicate bottle numbers. Check .CAP file."
        )
    ...
    if len(ssscc) > 5:
        log.warning(
            f"Length of {ssscc} name exceeds 5. Assigning STNNBR to {ssscc[0:3]}, CASTNO to {ssscc[3:5]}"
        )
